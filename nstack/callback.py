...
class PlotNeuralBDCallback(pl.Callback):
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, pl_module: NEURALBDModule):
        with torch.no_grad():
            ...
            for i in range(np.ceil(len(self.test_coords) / batch_size).astype(int)):
                ...
                output_img, convolved_imgs, psf, target_imgs, ref_img, ref_psfs = pl_module.model(batch_coordinates.to(pl_module.device))
            ...
            output_convolved_image = np.concatenate(output_convolved_image, 0).reshape(
                (512, 512, self.n_images, 2))
    ...
    def subplot_psf(self, ref_psf, pred_psf, plot_id: str):
        ...
        axs[1, 0].imshow(np.sqrt(ref_psf[:, :, 1]), origin='lower', vmin=0, vmax=1)
        axs[0, 1].imshow(np.sqrt(pred_psf[:, :, 0]), origin='lower', vmin=0, vmax=1)
        axs[1, 1].imshow(np.sqrt(pred_psf[:, :, 1]), origin='lower', vmin=0, vmax=1)
        axs[0, 0].set_title("Reference PSF")
        axs[0, 1].set_title("Predicted PSF")
        plt.tight_layout()
        wandb.log({plot_id: fig})
        plt.close(fig)
