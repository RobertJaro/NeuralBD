"""
Calculation of Karhunen-Loeve (KL) modes for Kolmogerov turbulence from
Asensio Ramos et al.: https://github.com/aasensio/neural-MFBD
"""
        ...
        for j in range(self.n_modes_max):
            ...
            for jpr in range(self.n_modes_max):
                ...
                if (deltaz):
                    ...
                    t2 = sp.gamma(14. / 3.0) * sp.gamma(11. / 6.0) ** 2 * (24.0 / 5.0 * sp.gamma(6.0 / 5.0)) ** (
                                5.0 / 6.0) / (2.0 * np.pi ** 2)

                    Kzz = t2 * t1 * phase

                    t1 = sp.gamma(0.5 * (n + npr - 5.0 / 3.0))
                    t2 = sp.gamma(0.5 * (n - npr + 17.0 / 3.0)) * sp.gamma(0.5 * (npr - n + 17.0 / 3.0)) * sp.gamma(
                        0.5 * (n + npr + 23.0 / 3.0))
