...
base_path = '/home/fs71254/schirni/neural_stacking'
...
fits_array = np.array(fits_array)

ref_image = fits_array[0]
ref_image = ref_image[offset:-offset, offset:-offset]
fits_array = fits_array[:, offset:-offset, offset:-offset]
vmin, vmax = np.min(ref_image), np.max(ref_image)

# plot images
#plt.figure(figsize=(20, 20))
#for i, img in enumerate(fits_array):
#    plt.subplot(10, 10, i + 1)
#    plt.imshow(img, cmap='gray', vmin=vmin, vmax=vmax, origin='lower')
#    plt.axis('off')
...
class ImageModel(nn.Module):
    ...
    def get_tranform(self, image_id, shift_range):
        ...
        theta = torch.tanh(transform[:, 2]) * np.pi * 0
...
positions_coordinates = np.ones((images.shape[0], images.shape[1], images.shape[2], 2))
positions_coordinates = (positions_coordinates) / (max_range - 1)
...
image_tensor = torch.from_numpy(images).float().reshape(-1, 1)
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        # plot full image
        test_coords = np.meshgrid(np.linspace(-max_range, max_range, images.shape[1]), np.linspace(-max_range, max_range, images.shape[2]), indexing='ij')
        ...
        for i in range(np.ceil(len(test_tensor) / batch_size).astype(int)):
            ...
            outputs += [output]
        output = np.concatenate(outputs).reshape(images.shape[1], images.shape[2])
